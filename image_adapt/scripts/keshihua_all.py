...
# 选择多个目标层
target_layers = [
    source_net.backbone.stages[3][-1].depthwise_conv,  # 最后一个stage
    source_net.backbone.stages[2][-1].depthwise_conv,  # 倒数第二个stage
    source_net.backbone.stages[1][-1].depthwise_conv,  # 倒数第三个stage
]
            ...
            if file.lower().endswith(('jpg', 'jpeg', 'png', 'bmp')):
                ...
                try:
                    ...
                    grayscale_cam = cam(
                        ...
                        targets=targets,
                        aug_smooth=True,  # 使用增强平滑
                        eigen_smooth=True  # 使用特征值平滑
                    )[0]

                    # 调整热力图的对比度
                    grayscale_cam = np.power(grayscale_cam, 1.3)  # gamma校正
                    ...
                    cam_image = show_cam_on_image(
                        ...
                        image_weight=0.5  # 调整原图与热力图的混合权重
                    )
...
img_dir = '/media/shared_space/yrzhen/data/ImageNet-C/gaussian_noise/5/n02085782/'
# img_dir = '/media/shared_space/dongxiaolin/maple1/data/imagenet/images/val/n02423022/'
# img_dir = '/media/shared_space/wuyanzu/ImageNet-C-Syn/gaussian_noise/5/n02085782/'
output_dir = '/media/shared_space/wuyanzu/ImageNet-C-Syn/++source_test'
